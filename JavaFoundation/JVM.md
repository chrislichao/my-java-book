...
---
##### 4，GC垃圾收集器
- 堆被划分了新生代和老年代，新生代主要使用复制和标记清除算法，老年代主要使用标记-整理垃圾回收算法，JVM针对新生代和老年代分别提供了多种不同的垃圾收集器
- Serial垃圾收集器（年轻代，单线程，复制算法）
- ParNew垃圾收集器（年轻代，Serial的多线程版本）
- Parallel Scavenge垃圾收集器（年轻代，多线程复制算法，高效）
- Serial Old垃圾收集器（老年代，单线程标记整理算法）
- Parallel Old垃圾收集器（老年代，多线程标记整理算法，Parallel Scavenge的老年代版本）
- CMS垃圾收集器（老年代，多线程标记清除算法）
- G1垃圾收集器
  - 基于CMS收集器，突出改进：
    - 1，基于标记整理算法，不产生内存碎片
    - 2，避免全区域垃圾收集，它把堆内存分为大小固定的几个独立区域，跟踪这些区域垃圾收集进度，同时后台维护优先级别表，每次根据允许的收集时间，优先回收垃圾最多的区域，提升垃圾收集效率
---
##### 5，类加载
- 类加载步骤
...
- 成员变量，局部变量，静态变量分别存放在哪？
  ...
  - 静态变量：static关键字修饰的变量
    ...
    - 静态变量实际创建的对象存放在堆中
- 什么是JVM分配担保机制
  - 当新生代（Eden区）无法分配内存时，会触发一次MinorGC，如果GC后依然无法分配，会启动分配担保机制，把新生代存活的对象直接转移到老年代，把新生代的空间腾出来放新对象，这里的老年代就是担保人；
...
- 什么是动态对象年龄判定机制
  - 当新生代（Eden区）无法分配内存时，会触发一次MinorGC，此时如果某Survivor区（from或者to）中年龄1+年龄2+...年龄N存活对象大小之和超过该Survivor区大小的一半，则该Survivor区中年龄大于等于N的存活对象会被直接转移到老年代中
---
##### 8，JVM性能调优
